...
// ─── Types (mirrored from mobile lib/hooks/useTasks.ts) ──────────────────────
...
// ─── Phase logic (mirrored from mobile) ──────────────────────────────────────
...
const SOURCE_BADGE: Record<string, { label: string; color: string }> = {
  vendor:        { label: 'Vendor',     color: '#4A7AB8' },
  vendor_custom: { label: 'Vendor',     color: '#4A7AB8' },
  setup:         { label: 'Profile',    color: '#8A7E6A' },
  documents:     { label: 'Documents',  color: '#7A9E7E' },
  countdown:     { label: 'Countdown',  color: '#C4756A' },
  budget:        { label: 'Budget',     color: '#C9A84C' },
  guests:        { label: 'Guests',     color: '#8A7E6A' },
}

// ─── Phase config ─────────────────────────────────────────────────────────────

const PHASE_CONFIG: Record<PhaseKey, { label: string; color: string; accent: string }> = {
  urgent: { label: 'Urgent',     color: '#C4756A', accent: 'rgba(196,117,106,0.08)' },
  soon:   { label: 'Coming up',  color: '#C9A84C', accent: 'rgba(201,168,76,0.06)'  },
  done:   { label: 'Completed',  color: '#7A9E7E', accent: 'rgba(122,158,126,0.06)' },
}
    ...
    <div style={{ marginBottom: 32 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 8 }}>
        ...
        <div style={{ fontSize: 13, color: '#F5EDD6' }}>
          ...
          <span style={{ color: '#5A5040', fontSize: 13 }}> / {total} tasks</span>
        </div>
      </div>
...
function TaskRow({ task, locale }: { task: Task; locale: string }) {
  ...
  const badge = task.source && task.source !== 'user' ? SOURCE_BADGE[task.source] : null
    ...
    <div>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 12, color: '#8A7E6A' }}>
          Read-only view — use the VELO app to add or complete tasks
          {lastSync && <span style={{ color: '#3A3830' }}> · synced {lastSync}</span>}
        </span>
      </div>
      ...
      {grouped.length === 0 && (
        <div style={{
          ...
          <div style={{ fontSize: 14, color: '#5A5040', marginBottom: 10 }}>No tasks yet</div>
      ...
      {/* Stats summary (desktop — useful when list is long) */}
      {grouped.length > 0 && (
        <div style={{
