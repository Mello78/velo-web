...
app/
  ...
  auth.tsx           # Login/registrazione + Google OAuth + Apple OAuth (iOS)
...
app/
  ...
  fornitori/page.tsx  # Lista vendor con filtri
...
components/
  ...
  SimpleNav.tsx       # Navbar per pagine interne (VA APPLICATA a fornitori e vendor)
...
**public_vendors** — include lat DOUBLE PRECISION e lng DOUBLE PRECISION.
I 24 vendor esistenti hanno coordinate popolate. I nuovi le ottengono via geocoding automatico (da implementare).

**couples** — partner1/2, wedding_date, budget, nationality, country_of_origin,
wedding_region, wedding_city, wedding_style, wedding_season, ceremony_type,
wedding_size, color1, color2

**vendor_accounts** — lat, lng, max_events_per_day, work_regions[], logo_url

**Altre tabelle:** tasks, guests, expenses, messages, vendors, custom_vendors,
vendor_milestones, vendor_blocked_dates, vendor_invites, admin_users
...
2. BOM in Cursor — se non funziona, ricrea con PowerShell New-Item
3. Import Supabase app -> ../lib/supabase (un solo ..)
4. Next.js App Router — lingua da cookie NEXT_LOCALE con cookies() di next/headers
5. Nominatim — header User-Agent: VELOWedding/1.0 obbligatorio, max 1 req/sec
6. Commit sito -> sempre dalla cartella velo-web-temp
...
- Google OAuth: configurato su Supabase (provider Google abilitato)
